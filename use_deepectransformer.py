...
def use_deepectransformer(name):
    ...
    os.system(f'conda run -n deepectransformer python {path}/DeepProZyme/run_deepectransformer.py -i {path}/working/{name}/{name}_homoleft.fasta -o {path}/working/{name}/{name}_deepec_result -g cpu -b 128 -cpu 2')
    os.chdir(f"{path}")
