...
import align2
def foldseek_choose(refname,refmodel,name):
    ...
    for i in range(len(gx.index)):
        if model_transpoter_found.transpoter(gx.iat[i, 1]) == 1:
            if gx.iat[i, 5] >= 0.9 and gx.iat[i, 6] > 0.9 and gx.iat[i, 7] > 70 and gx.iat[i, 8] > 70 and gx.iat[i, 4] >= 0.85 and align2.align(gx.iat[i,1],gx.iat[i,2],name, refname)>=0.5:
...
def foldseek_choose2(refname,refmodel,name):
    foldseek_choose(refname,refmodel,name)