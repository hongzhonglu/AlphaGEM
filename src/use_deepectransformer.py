...
def use_deepectransformer(name):
    path = os.getcwd()
    os.chdir(f"{path}/tools/DeepProZyme")
    #activate_deepectransformer_env()
    os.system(f'conda run -n deepectransformer python {path}/tools/DeepProZyme/run_deepectransformer.py -i {path}/working/{name}/{name}_homoleft.fasta -o {path}/working/{name}/{name}_deepec_result -g cpu -b 128 -cpu 2')
    os.chdir(f"{path}")
