import cobra
import pandas as pd
modelyeast=cobra.io.read_sbml_model('models/universal-GEM_fixed_final.xml')
...
iml=cobra.io.load_json_model('models/iML1515_old.json')
chebi2metdict1={}
keggdict={}
...
for met in modelyeast.metabolites:
    try:
        met.id='chebi'+met.annotation['chebi'][0].split(':')[-1]+'_'+met.compartment
    except:
        count+=1
        mets1.append(met)
...
for met in mets1:
    try:
        met.id='chebi'+chebi2metdict1[met.annotation['MetaNetX (MNX) Chemical'].split('/')[-1]]+'_'+met.compartment
    except:
        count2+=1
        mets2.append(met)
count3=0
...
for met in mets2:
    try:
        met.id='chebi'+chebi2keggdict[met.annotation['KEGG Compound'].split('/')[-1]]+'_'+met.compartment
    except:
        print(met)
        count3+=1
        mets3.append(met)
...
mappingdict={}
count=0
for r in list(modelyeast.reactions):
    if len(list(r.compartments))>=2:
        continue
    try:
        print(r.annotation['rhea'])
        r.id='rhea_'+r.annotation['rhea'][0].split('/')[-1]+'_'+list(r.compartments)[0]
    except:
        pass
#     try:
#         if True:
#            mappingdict[r.id]=rhea2metanetxdict[r.annotation['metanetx.reaction']][:-2]
...
#                 mappingdict[r.id] =rhea2keggdict[r.annotation['kegg.reaction']][:-2]
#             else:
#                 count += 1
#                 reacts.append(r)
#         except:
...
# my_dict = mappingdict
# # Save to file
# with open('panyeast2rhea.pkl', 'wb') as f:  # 'wb' = write binary
#     pickle.dump(my_dict, f)        # Serialize and save dict
#
# # Load from file
# with open('data.pkl', 'rb') as f:  # 'rb' = read binary
#     loaded_dict = pickle.load(f)   # Deserialize data
